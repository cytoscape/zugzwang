...
import static org.cytoscape.zugzwang.internal.visualproperties.Position.CENTER;
import static org.cytoscape.zugzwang.internal.visualproperties.Justification.JUSTIFY_CENTER;
...
{	
	...
	public boolean equals(Object lp) 
	{
		...
		if (Math.abs(p.getOffsetX() - xOffset) > 1e-7)
			return false;
	
		if (Math.abs(p.getOffsetY() - yOffset) > 1e-7)
			return false;
	...
	public Vector2 calcOffset(Vector4 metricsObject, Vector4 metricsTarget)
	{
		Vector2 posObject = calcPosition(metricsObject, objectAnchor);
		Vector2 posTarget = calcPosition(metricsTarget, targetAnchor);
		
		Vector2 delta = Vector2.subtract(posTarget, posObject);
		delta.x += xOffset;
		delta.y += yOffset;
	...
	/**
	 * Calculates the position of an anchor based on a bounding box and the anchor's type
	 * 
	 * @param bounds Bounding box: minX, minY, maxX, maxY
	 * @param pos Anchor type
	 * @return Anchor position relative to the center of the bounding box
	 */
	private static Vector2 calcPosition(Vector4 bounds, Position pos)
	{
		...
		switch (pos)
		{
			...
			case NORTH:
				result.x = (bounds.x + bounds.z) * 0.5f;
				result.y = bounds.y;
			break;
			...
			case WEST:
				result.x = bounds.x;
				result.y = (bounds.y + bounds.w) * 0.5f;
			break;
