package org.cytoscape.zugzwang.internal.icons;
...
import org.cytoscape.zugzwang.internal.arrowshape.ZZArrowShape;
...
public class VisualPropertyIconFactory {	
	...
	public static <V> Icon createIcon(V value, int w, int h) {
		...
		{
			final ZZNodeShape dShape;
			if(NodeShapeVisualProperty.isDefaultShape((NodeShape) value))
				dShape = ZZNodeShape.getZZShape((NodeShape) value);
			else
				dShape = (ZZNodeShape)value;
			icon = new NodeIcon(dShape.getShape(), w, h, dShape.getDisplayName());
		} 
		else if(value instanceof LineType) 
		{
			icon = new StrokeIcon(ZZLineType.getZZLineType((LineType) value).getStroke(2f), w, h, value.toString());
		} 
		...
		else if(value instanceof ArrowShape) 
		{
			...
			final ZZArrowShape dShape;
			if(ArrowShapeVisualProperty.isDefaultShape(arrowShape))
				dShape = ZZArrowShape.getArrowShape(arrowShape);
			else
				dShape = ZZArrowShape.NONE;
